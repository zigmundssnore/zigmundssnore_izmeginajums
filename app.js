...
function buildGallery() {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    sAvif.type = 'image/avif'; sAvif.srcset = `photo${i}.avif`;
    const sWebp = document.createElement('source');
    sWebp.type = 'image/webp'; sWebp.srcset = `photo${i}.webp`;
    const img = document.createElement('img');
    img.src = `photo${i}.jpg`; img.alt = `Art piece ${i}`;
    ...
    picture.appendChild(sAvif); picture.appendChild(sWebp); picture.appendChild(img);
    a.appendChild(picture); frag.appendChild(a);
  }
...
function createParticles() {
  ...
  let rafId = null, paused = preferReduced;
  ...
  const particles = Array.from({ length: preferReduced ? 30 : 120 }, () => ({
    ...
    o: Math.random()*0.6+0.2, vx: (Math.random()-0.5)*0.15, vy: - (Math.random()*0.25+0.05),
    c: colors[(Math.random()*colors.length)|0], flicker: Math.random()*0.02+0.005
  }));
  const draw = () => {
    ...
    for (const p of particles) {
      ...
      ctx.beginPath();
      ctx.fillStyle = p.c; ctx.globalAlpha = Math.max(0.08, Math.min(0.8, p.o));
...
function init() {
  initYear(); initNav(); buildGallery(); createLightbox('#gallery'); revealSections();
